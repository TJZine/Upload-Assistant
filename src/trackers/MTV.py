...
class MTV:
    ...
    async def upload(self, meta):
        ...
        if (
            ...
            console.print(
                "[red]Piece size is OVER 8M and does not work on MTV. Generating a new .torrent"
            )
            from src.prep import Prep

            prep = Prep(
                screens=meta["screens"], img_host=meta["imghost"], config=self.config
            )
        ...
        await common.edit_torrent(
            meta, self.tracker, self.source_flag, torrent_filename=torrent_filename
        )
        ...
        if not all(
            any(x in image["raw_url"] for x in approved_imghosts)
            for image in meta["image_list"]
        ):
            console.print(
                "[red]Unsupported image host detected, please use one of the approved imagehosts"
            )
            return
        # getting description
        ...
        files = {"file_input": (f"{meta['name']}.torrent", tfile)}
                ...
                try:
                    if "torrents.php" in response.url:
                        ...
                        if "authkey.php" in response.url:
                            console.print(
                                f"[red]No DL link in response, So unable to download torrent but It may have uploaded, go check"
                            )
                            print(response.content)
                            console.print(
                                f"[red]Got response code = {response.status_code}"
                            )
                            print(data)
                        else:
                            console.print(
                                f"[red]Upload Failed, Doesnt look like you are logged in"
                            )
                            print(response.content)
                            print(data)
                except:
    ...
    async def edit_desc(self, meta):
        base = open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/DESCRIPTION.txt", "r"
        ).read()
        with open(
            f"{meta['base_dir']}/tmp/{meta['uuid']}/[{self.tracker}]DESCRIPTION.txt",
            "w",
    ...
    async def edit_name(self, meta):
        ...
        if meta["source"].lower().replace("-", "") in mtv_name.replace("-", "").lower():
            ...
            mtv_name = meta["name"]
            if (
                meta.get("type") in ("WEBDL", "WEBRIP", "ENCODE")
                and "DD" in meta["audio"]
            ):
                mtv_name = mtv_name.replace(
                    meta["audio"], meta["audio"].replace(" ", "", 1)
                )
    ...
    async def get_tags(self, meta):
        ...
        for each in ["remux", "WEB.DL", "WEBRip", "HDTV", "BluRay", "DVD", "HDDVD"]:
            if (each.lower().replace(".", "") in meta["type"].lower()) or (
                each.lower().replace("-", "") in meta["source"]
            ):
                tags.append(each)
    ...
    async def validate_credentials(self, meta):
        ...
        if vcookie != True:
            console.print(
                "[red]Failed to validate cookies. Please confirm that the site is up and your username and password is valid."
            )
        ...
        if vapi != True:
            console.print(
                "[red]Failed to validate API. Please confirm that the site is up and your API key is valid."
            )
        return True
    ...
    async def login(self, cookiefile):
        with requests.Session() as session:
            ...
            if resp.url.endswith("twofactor/login"):
                ...
                resp = session.post(
                    url="https://www.morethantv.me/twofactor/login",
            ...
            if "authkey=" in resp.text:
                ...
                console.print(
                    "[bold red]Something went wrong while trying to log into MTV"
                )
        ...
        try:
            ...
            if rr is not None:
                ...
                if "status_message" in rr:
                    ...
                    console.print(
                        f"[red]Site Seems to be down or not responding to API"
                    )
        except:
            console.print(
                f"[red]Unable to search for existing torrents on site. Most likely the site is down."
            )
